...
- автоматическая смена HomeProxy-ноды, если YouTube перестал отвечать;
...
### Автоматический подбор ноды

Скопируйте [`.env_example`](.env_example) в `/root/.env` на роутере и замените пример URL своим. URL не нужно передавать в командной строке:

```sh
VPN_SUBSCRIPTION_URL='https://provider.example/subscription/token'
```

Защитите файл правами `chmod 600 /root/.env` и не добавляйте его в Git. При запуске из каталога с `.env` скрипт возьмёт этот файл; иначе использует `/root/.env`. Другой путь можно указать через `--env-file`.

```sh
vpn auto --output /root/subscription.sub --env-file /root/.env
```

Если HTTPS-запрос к YouTube через локальный `mixed`-вход HomeProxy проходит, команда ничего не меняет. Иначе она сохраняет исходную конфигурацию HomeProxy, останавливает сервис, скачивает и проверяет подписку, затем запускает обычный benchmark **для всех нод**. Только ноды, прошедшие проверку через установленный sing-box, пробуются в HomeProxy в порядке скорости. После каждой ноды повторяется запрос к YouTube через `mixed`-вход; первая успешная остаётся активной. Номера относятся к **валидным** нодам подписки и могут измениться при обновлении списка. `--max N` ограничивает только число применений, но не число проверяемых нод (по умолчанию попытки идут до конца списка). `--section` и `--routing-node` работают как у `homeproxy-apply`. Если ничего не подошло или произошла ошибка, исходная конфигурация восстанавливается и HomeProxy перезапускается. Создаётся один backup `homeproxy.bak-auto-*`, а не backup на каждую ноду.

Порт `mixed`-входа берётся из `/var/run/homeproxy/sing-box-c.json`; DNS имени YouTube выполняется через HomeProxy, а не системный DNS роутера. Если такого входа нет, `auto` завершится до изменения конфигурации. При правилах, которые по-разному обрабатывают `mixed`-вход и устройства LAN, результат всё ещё может отличаться от браузера на компьютере. Benchmark запускает временный экземпляр **уже установленного** sing-box для каждой ноды по очереди; IPv6 не меняется.
