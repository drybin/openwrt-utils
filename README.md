...
Если HTTPS-запрос к YouTube с роутера проходит, команда ничего не меняет. Иначе она сохраняет исходную конфигурацию HomeProxy, останавливает сервис, скачивает и проверяет подписку, затем запускает обычный benchmark **для всех нод**. Только ноды, прошедшие проверку через установленный sing-box, пробуются в HomeProxy в порядке скорости. После каждой ноды повторяется запрос к YouTube; первая успешная остаётся активной. Номера относятся к **валидным** нодам подписки и могут измениться при обновлении списка. `--max N` ограничивает только число применений, но не число проверяемых нод (по умолчанию попытки идут до конца списка). `--section` и `--routing-node` работают как у `homeproxy-apply`. Если ничего не подошло или произошла ошибка, исходная конфигурация восстанавливается и HomeProxy перезапускается. Создаётся один backup `homeproxy.bak-auto-*`, а не backup на каждую ноду.

Проверка YouTube выполняется **с самого роутера**. При правилах HomeProxy, которые по-разному обрабатывают трафик роутера и устройств LAN, результат может отличаться от браузера на компьютере. Benchmark запускает временный экземпляр **уже установленного** sing-box для каждой ноды по очереди; IPv6 не меняется.
